    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          path: { 
            type: "string", 
            description: "The route path (e.g., /properties, /financial)" 
          }
        },
        required: ["path"]
      }
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          type: { 
            type: "string", 
            enum: ["Financial", "Operational", "Market", "Compliance"] 
          },
          period: { 
            type: "string", 
            description: "e.g., November 2024, Q3 2024" 
          },
          summary: { 
            type: "string", 
...
const getSystemContext = (path: string): string => {
  ...
  3. Always provide helpful, actionable insights.
  `;
};
...
export const generateAIResponse = async (
  ...
  try {
    ...
    // Handle Tool Calls (Generative UI)
    const toolCalls = choice.message?.tool_calls;
    
    if (toolCalls && toolCalls.length > 0) {
      const call = toolCalls[0]; // Handle primary tool call
      if (call && call.function) {
        ...
        } else if (functionName === 'show_alerts') {
          uiPayload = { type: 'alert_list', data: args['items'] || [] };
        } else if (functionName === 'request_approval') {
        ...
        if (!text) {
          text = `I've generated the ${functionName.replace(/_/g, ' ')} you requested.`;
        }
      }
    }

    if (!text && !uiPayload) throw new Error("Empty response from AI model");

    return { text, uiPayload };

  } catch (error) {
    console.error("GitHub Model Chat Error:", error);
    return simulateStructuredResponse(userMessage, context);
  }
};
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are a strategic real estate asset management analyst. 
          Analyze the request and provide insights in JSON format with these fields:
          - title: A concise, catchy title for the insight
          - explanation: Array of 2-3 bullet points explaining the data trend or issue
          - prediction: A forward-looking prediction based on the data
          - suggestions: Array of 3 actionable suggestions for the user`
        },
        {
          role: "user",
    ...
    console.error("GitHub Model Insight Error:", error);
    return simulateInsightResponse(prompt);
  }
};
